...
def DELETE_STUDENT(request,admin):
    ...
    student.delete()
    messages.success(request,'Student data has been successfully deleted')
    return redirect('view_student')
...
def ADD_SESSION(request):
    if request.method == 'POST':
        ...
        session.save()
        messages.success(request,'Session has been successfully added')
...
def STAFF_DISAPPROVE_LEAVE(request,id):
    ...
    leave.status = 2
    leave.save()
    messages.success(request,'Leave has been disapproved')
    return redirect('staff_leave_view')
